...
\newcommand{\NOTE}[1]{\par\smallskip\noindent\fcolorbox{red!55}{red!7}{\parbox{0.965\linewidth}{\footnotesize\textcolor{red!70!black}{\textbf{NOTE.}}~#1}}\par\smallskip}
...
\maketitle
\thispagestyle{empty}
\NOTE{\textbf{Overview / open items.} Draft is structurally complete and compiles. Before submission: (1) de-duplicate subset figures --- keep \emph{one} representation per result (flagged inline); (2) reconcile the headline tables (fresh $dt{=}0.003$) with the appendix tables (report $dt{=}0.005$) --- pick one $dt$; (3) decide bullet vs.\ prose for the target venue; (4) add related-work comparison (LMHE, noisyTS Fig.~3); (5) author list is empty.}
\vspace{-2.2em}

% =====================================================================
\section*{Abstract}
\NOTE{Bullet ``Highlights'' abstract --- confirm the venue allows it; many require a single prose paragraph. Numbers here are the fresh $dt{=}0.003$ test-287 values (45.3\% vs.\ 23.7\% at 200\,pm).}
% =====================================================================
\noindent
...
\NOTE{Intro is in bullets --- most journals expect prose. Add one line that HIP is \emph{held out} (not trained on Transition1x) and why T1x is the right benchmark.}% =====================================================================
\begin{itemize}
...
\NOTE{Pin the headline GAD config: the figures/headline tables use the fresh $dt{=}0.003$ Cartesian IRC, but the text calls $dt{=}0.005$ ``representative.'' State which $dt$ the headline numbers are and keep it consistent.}
% =====================================================================
...
\NOTE{Figs.~\ref{fig:primary} and~\ref{fig:delta} are two views of the \emph{same} datapoint (intended success). You asked for one representation per result --- recommend the joint intended+convergence figure (\texttt{figures\_new/fig\_intended\_success.pdf}) and dropping the single/delta pair.}
...
\end{figure}
\NOTE{Subset duplicate of Fig.~\ref{fig:outcome} (same data, counts vs.\ \%). Keep one.}

\subsection{Mechanism: it is not wrong basins --- it is one-sided saddles and non-convergence}\label{sec:mech}
\NOTE{Strongest, most honest section --- now fully data-correct (unintended $\approx$ 0). Good. Consider opening the Discussion with this reframing.}
...
\label{fig:grouped}
\end{figure}
\NOTE{Third view of the outcome data (Figs.~\ref{fig:outcome},~\ref{fig:outcomecounts}). Drop unless it earns its place.}

\subsection{Secondary: saddle-convergence and the \texorpdfstring{$\fmax$}{Fmax} plateau}\label{sec:fmax}
\NOTE{Two plateau figures (2-panel + grid) downstream --- keep one. The plateau mechanism + the 10k-budget ``intrinsic'' check are solid; keep both points.}
...
\NOTE{Three cost views (wall-time, steps, pareto). Pareto is the joint one; steps duplicates wall-time --- trim to two.}
...
\NOTE{Good to state the scope boundary. Caveat the numbers: reactant is convergence-only (no IRC) and partial-$n$ for GAD; midpoint GAD/hybrid IRC is not yet aggregated.}
...
% =====================================================================
\section{Discussion}
\NOTE{Add explicit comparison to LMHE (ML leftmost-eigenvector) and the noisyTS Fig.~3 positioning (0--15\,pm) --- a reviewer will expect both, currently absent.}
% =====================================================================
\begin{itemize}
...
\NOTE{Pointers are good. For camera-ready add the GitHub URL + the exact commit hash.}
{\sloppy
\begin{itemize}
...
\caption{\textbf{All-method four-axis summary (joint).} Intended-reaction success, saddle-convergence, wall-time per converged TS, and RMSD-to-true-TS, all nine configurations plotted against initial-guess noise on one set of axes ($n=287$). The single most complete view of the benchmark.}
\label{fig:gal0}
\end{figure}
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{figures_new/fig_intended_success.pdf}
\caption{\textbf{Primary result (focused).} Intended-reaction success (left) and saddle-convergence (right) vs.\ noise for GAD, the hybrid, and the strongest Sella baseline. The methods tie when the guess is good and GAD pulls ahead as it degrades; the crossover holds on both axes.}
\label{fig:gal1}
\end{figure}
\clearpage
\begin{figure}[htbp]\centering
...
\caption{\textbf{Cost vs.\ chemical reliability (joint).} Wall-time per converged TS against intended success, one point per noise level per method. The hybrid is Pareto-dominant at high noise.}
\label{fig:gal4}
\end{figure}
\begin{figure}[htbp]\centering
