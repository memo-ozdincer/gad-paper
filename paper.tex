...
primary metric is the IRC-validated \emph{intended success rate} (forward and
...
\item \textbf{Crossover, not a point.} GAD is statistically indistinguishable
  ...
  45.3\% vs.\ 23.7\%). The same crossover holds for convergence and across
  \fmax{} thresholds.
...
\noindent\textbf{Metrics --- primary vs.\ secondary.}
\begin{itemize}
\item \textbf{Convergence rate (secondary):} optimizer reached a first-order
  saddle, $\nneg{=}1 \wedge \fmax{<}\tau$ (default $\tau=0.01\evA$; threshold
  family in Appendix~\ref{app:crit}). Necessary but \emph{insufficient}.
\item \textbf{Intended success rate (primary):} after optimization, follow
  Sella's IRC \cite{sella2022} forward/backward and graph-match each endpoint
  (Open~Babel connectivity + VF2) to the labelled reactant/product. Outcomes
  (after \cite{newtonnet,lmhe}): \emph{intended} (both endpoints),
  \emph{partially intended} (one), \emph{unintended} (valid saddle, other
  reaction), \emph{no reaction} (collapsed to a minimum), \emph{TS error}
  (no convergence).
\end{itemize}
...
\section{Results}
% =====================================================================
\subsection{Primary: intended success rate crosses over with guess quality}\label{sec:primary}

\begin{figure}[H]\centering
\includegraphics[width=0.93\textwidth]{figures_new/fig_intended_success.pdf}
\caption{\textbf{Primary (a) and secondary (b) axes vs.\ noise}, $n=287$.
Methods coincide at low noise; GAD and the hybrid retain far more intended
reactions as the guess degrades.}
\label{fig:primary}
\end{figure}
...
\item \textbf{Crossover.} Tie at low noise (intended 88--89\% at \SI{10}{pm};
  Sella even leads on raw \emph{convergence}, 92.7 vs.\ 89.2\%); GAD pulls ahead
  with degradation: $\Delta$intended $=+3.9,+11.5,+21.6$~pp at 100/150/\SI{200}{pm}
  (Fig.~\ref{fig:delta}, Table~\ref{tab:headline}).
\item \textbf{Significance.} McNemar on paired per-sample intended outcomes:
  ...
  \SI{200}{pm} ($p{=}1.7\!\times\!10^{-13}$). Same crossover on convergence and
  across \fmax{} thresholds.
\end{itemize}

\begin{table}[H]\centering\small
\caption{Intended success rate (primary) and convergence rate (secondary), \%,
$\fmax<0.01$, $n=287$. Intended from the fresh per-sample IRC re-validation
(GAD $dt{=}0.003$); Sella $=$ strongest baseline.}
\label{tab:headline}
\begin{tabular}{ll cccccc}
\toprule
& & \textbf{10\pm} & \textbf{30\pm} & \textbf{50\pm} & \textbf{100\pm} & \textbf{150\pm} & \textbf{200\pm} \\
\midrule
\multirow{2}{*}{\textbf{Intended \%}}
...
\subsection{Secondary: the \texorpdfstring{$\fmax$}{Fmax} plateau (and why it does not matter for chemistry)}\label{sec:fmax}
...
\caption{Intended success rate \% (top) and convergence rate \% (bottom), all
